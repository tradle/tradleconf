...
Conf.prototype.destroy = co.wrap(function* (opts) {
  ...
  const buckets = yield utils.listStackBuckets(this.aws, stackName)
...
Conf.prototype.promptFunction = co.wrap(function* (message) {
  ...
  logger.info()
  const functions = yield utils.listStackFunctions(aws, stackName)
  const shortNames = functions.map(f => f.PhysicalResourceId.slice(stackName.length + 1))
