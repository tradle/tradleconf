...
const { debug, prettify, isValidProjectPath, toEnvFile } = utils
...
const functions = {
  setconf: 'setconf',
  cli: 'cli',
  createDataBundle: 'import_data_bundle',
  createDataClaim: 'create_data_claim',
  getDataBundle: 'get_data_bundle'
}
...
Conf.prototype.getDeployItems = function getDeployItems (opts) {
  ...
  if (opts.all || opts.terms) {
    parts.terms = read.terms()
  }

  if (opts.all || opts.models) {
...
Conf.prototype.createDataBundle = co.wrap(function* ({ path }) {
  ...
  return yield this.invokeAndReturn({
    functionName: functions.createDataBundle,
    arg: bundle
  })
})

Conf.prototype.createDataClaim = co.wrap(function* ({ key }) {
  if (!key) throw new Error('expected "key"')

  return yield this.invokeAndReturn({
    functionName: functions.createDataClaim,
    arg: { key }
  })
})

Conf.prototype.getDataBundle = co.wrap(function* ({ claimId }) {
  if (!claimId) throw new Error('expected "claimId"')

  return yield this.invokeAndReturn({
    functionName: functions.getDataBundle,
    arg: { claimId }
  })
})
