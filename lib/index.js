...
Conf.prototype.init = co.wrap(function* () {
  ...
  const stackInfos = yield this.getStacks(awsProfile)
...
Conf.prototype._getAWSClient = function (profile) {
  if (!profile) profile = process.env.awsProfile
  if (profile) {
...
Conf.prototype._remoteOnly = function () {
  if (this.local) {
    throw new CustomErrors.InvalidOptions('not supported for local dev env')
  }
}
...
Conf.prototype.destroy = co.wrap(function* (opts) {
  ...
  const { stackName, awsProfile } = process.env
  ...
  const aws = this._getAWSClient()
  const buckets = yield utils.listStackBuckets(aws, stackName)
  ...
  for (const id of buckets) {
    ...
    utils.destroyBucket(aws, id)
  }

  yield utils.deleteStack(aws, stackName)
})
...
Conf.prototype.info = co.wrap(function* () {
  ...
  return {
    apiBaseUrl: yield utils.getApiBaseUrl(this._getAWSClient(), process.env.stackName),
    links: links.result
  }
})
