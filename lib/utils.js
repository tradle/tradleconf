...
const listStackResources = co.wrap(function* (aws, StackName, filter=acceptAll) {
  let resources = []
...
const listStackResourceIdsByType = co.wrap(function* (type, aws, stackName) {
  const resources = yield listStackResourcesByType(type, aws, stackName)
